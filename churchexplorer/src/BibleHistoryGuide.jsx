...
const BibleHistoryGuide = ({ onNavigate, onGoBack, onProgressUpdate }) => {
  ...
  useEffect(() => {
    if (location.state?.reviewMode && location.state.reviewMode.path === 'bible') {
  ...
  const getMasteryLevel = (completed, total) => {
    ...
    if (percentage < 38) return 'Novice';
    if (percentage < 63) return 'Scholar';
    if (percentage < 88) return 'Expert';
    return 'Master';
  };
