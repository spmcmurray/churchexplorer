...
import { CheckCircle, ChevronDown, ChevronRight, Heart, Lock, ArrowLeft } from 'lucide-react';
...
const GospelMessageGuide = ({ userProgress, onNavigate, onGoBack, onProgressUpdate }) => {
  ...
  useEffect(() => {
    if (location.state?.reviewMode && location.state.reviewMode.path === 'gospelMessage') {
  ...
  const handleLessonComplete = async (lessonNumber, xp) => {
    ...
    await completeCourseLesson(user.uid, 'gospelMessage', lessonNumber, xp);
    await scheduleReviews(user.uid, 'gospelMessage', lessonNumber);
      ...
      <div className="relative overflow-hidden text-white py-16 px-4">
        ...
        <div className="relative max-w-4xl mx-auto z-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-4xl md:text-5xl font-black">What Is the Gospel?</h1>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="space-y-4">
          {lessons.map((lesson, index) => {
            ...
            return (
              <div key={lesson.number} className={`bg-white rounded-2xl border-2 transition ${isLocked ? 'border-slate-200 opacity-60' : 'border-slate-200 hover:border-red-300'}`}>
                <div className="p-6 cursor-pointer" onClick={() => !isLocked && setExpandedLesson(isExpanded ? null : lesson.number)}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4 flex-1">
                      {isLocked ? <Lock className="w-6 h-6 text-slate-400" /> : isCompleted ? <CheckCircle className="w-6 h-6 text-red-600" /> : <div className="w-6 h-6 rounded-full border-2 border-slate-300" />}
                      <div className="flex-1">
