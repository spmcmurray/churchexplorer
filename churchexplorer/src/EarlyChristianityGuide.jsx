...
const EarlyChristianityGuide = ({ userProgress, onNavigate, onGoBack, onProgressUpdate }) => {
  ...
  useEffect(() => {
    if (location.state?.reviewMode && location.state.reviewMode.path === 'earlyChristianity') {
  ...
  const handleLessonComplete = async (lessonNumber, xp) => {
    ...
    await completeCourseLesson(user.uid, 'earlyChristianity', lessonNumber, xp);
    await scheduleReviews(user.uid, 'earlyChristianity', lessonNumber);
      ...
      <div className="relative overflow-hidden text-white py-16 px-4">
        ...
        <div className="relative max-w-4xl mx-auto z-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-4xl md:text-5xl font-black">What Early Christians Believed</h1>
              <p className="text-rose-100">Before denominations and how the Trinity formed</p>
            </div>
          </div>
        </div>
      </div>
            ...
            return (
              <div
                ...
                className={`bg-white rounded-2xl border-2 transition ${
                  isCompleted ? 'border-rose-400' : isLocked ? 'border-slate-200 opacity-60' : 'border-slate-200 hover:border-rose-300'
                }`}
              >
                <div 
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4 flex-1">
                      <div className={`w-12 h-12 min-w-[3rem] rounded-full flex items-center justify-center text-xl font-bold flex-shrink-0 ${
                        isCompleted ? 'bg-rose-100 text-rose-700' : isLocked ? 'bg-slate-200 text-slate-400' : 'bg-rose-100 text-rose-700'
                      }`}>
                  ...
                  <div className="border-t-2 border-slate-100 p-6">
                    <button
                      ...
                      className={`w-full py-3 px-6 rounded-xl font-semibold transition ${
                        lesson.data 
                          ? 'bg-gradient-to-r from-rose-500 to-pink-500 text-white hover:shadow-lg cursor-pointer'
